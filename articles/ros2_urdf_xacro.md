---
title: "ROS2で自作ロボットをRviz上に3D表示してみた【URDF/Xacro入門】"
emoji: "🤖"
...
今回はRviz（ROS Visualization）で設計したロボットを3D表示する環境の構築方法についてまとめます。環境の構築方法を理解できれば、自作のロボットをROS環境に取り込んで地図作成＋自己位置推定（SLAM）や自律走行（Nav2）への応用ができるため、記事としてアウトプットします。
...
ROS2自体の環境構築方法および必要パッケージのインストールについてはこの記事では割愛します。なお、自分の場合はWSL2 + Docker（Dev Container）でROS2 Humbleの環境を構築しています。
...
`ros2_ws/src` ディレクトリに移動後、一からパッケージを作成し、必要なディレクトリを作成します。
...
今回の `robot_description` パッケージはURDFを表示するだけのシンプルな構成なので、以下の内容に修正します。
...
`package.xml` にはパッケージの情報・依存関係・ライセンスを記述します。実行時に必要なパッケージ（`xacro`、`rviz2` など）もここで宣言します。
...
<package format="3">
  ...
  <maintainer email="XXXXXXXXXXXXXX">XXXXX</maintainer>
...
1. **robot_state_publisher** — launchファイルで展開したURDFを受け取り `/robot_description` に公開する。`/joint_states` を購読してジョイントの状態を`tf`としてブロードキャストする
...
![Rviz初期表示](/images/ros_urdf/image-4.drawio.png)
...
前回のライトローバーの記事でも触れていますが、**tfは「ある座標系から見た別の座標系の位置と向き」を表します**。今回配信するのは `world → base_link` のtfです。`world` は固定された基準座標系（原点）、`base_link` はロボット中心の座標系なので、`tf`を配信することで原点からのロボットの位置・姿勢を表現できます。
